...
* **DC Bus Undervoltage Cut-Out (backplane logic rail):** The AP64352 5 V buck's EN pin sits on a 100 kΩ / 6.19 kΩ divider from VIN, programming its undervoltage lockout (datasheet Eq. 2/3 with the 1.5 µA / 5.5 µA EN source currents): the 5 V rail turns on at ≈20.1 V rising (18.9–21.3 V over part tolerance) and drops out at ≈18.1 V falling (17.1–19.2 V). When it drops out the TCA9539 loses VCC and its port pins go high-impedance; each blade's EN (B16) is then held by the blade's own 100 kΩ pull-down against the MPQ4242's 6 µA internal EN pull-up, about 0.6 V against a 1.2 V falling threshold, so every port switches hard off. The management card reboots with the rail (its 3.3 V LDO hangs off the same 5 V), and on recovery the expander powers up with its ports as inputs, so no blade re-enables until firmware applies the boot policy. This hardware cut-out, not the budget engine, is the ultimate low-bus failsafe: a sagging supply cannot be dragged below ~18 V with the ports still loaded, which bounds the worst bus current at roughly 35 A (six ports at 100 W just before the cut) — under the 40 A fuse and within the XT60's burst rating. The visible symptom of a supply that cannot hold 20 V under load is a brownout reboot loop rather than a hazard; the staggered boot re-enable limits the re-applied step.
