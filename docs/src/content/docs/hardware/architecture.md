...
| **PROBE** | Blade seated (at boot, the seated blades are released into PROBE one at a time, 250 ms apart, in priority order). | One attempt per tick: select mux channel, probe/configure INA226 (alert at 125% of the port's current limit, latched), probe/configure MPQ4242 (GPIO fns, peak CL, CC blank time, spread-spectrum dither, 5 Fixed + 2 PPS PDO set, PDO current ceiling), assert EN. 3 failures → FAULT. | Blinking Cyan (2 Hz) | → IDLE on success. → FAULT on repeated probe failure. |
